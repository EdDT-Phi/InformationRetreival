...
public abstract class Document {
   ...
   */
  protected static final String stopWordsFile = "ir/utilities/stopwords.txt";
  /**
  ...
  public HashMapVector hashMapVector(HashSet<String> bigrams) {
    ...
    while (hasMoreTokens()) {
      String token = nextToken();
      if(bigrams.contains(prev +" " + token)){
        vector.increment(prev +" " + token);
        prev = nextToken();
      }
      else {
        vector.increment(prev);
        prev = token;
      }
    }
